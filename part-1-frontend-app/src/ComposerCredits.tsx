...
function ComposerCredits(props: ComposerCreditsProps) {
  const [credits, setCredits] = useState<CrewCredit[]>([])
  const [loaded, setLoaded] = useState(false)
  const [error, setError] = useState<Error | null>(null)

  useEffect(() => {
    fetchCredits(props.personId).then(setCredits).then(() => setLoaded(true)).catch(setError)
  }, [])
  ...
  const otherCredits = credits
    .filter((credit) => credit._embedded.show.id !== props.showId)
    .sort(byShowPremiere)
